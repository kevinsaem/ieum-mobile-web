...
- 사용자는 관리자가 발급한 6자리 사용자 번호와 6자리 이상 비밀번호로 로그인합니다. 역할은 선택하지 않고 서버가 자동 확인합니다.
- 기존 연동 호환을 위해 API는 `email` 요청 필드도 허용하며, 번호가 아직 발급되지 않은 사용자는 모바일 로그인 도움말의 `기존 이메일로 로그인` 보조 경로를 사용할 수 있습니다.
- 정규화된 계정 식별자 기준 5회 또는 IP 기준 30회 요청이 5분 창에서 누적되면 HTTP 429와 `Retry-After`를 반환합니다.
- 계정 식별자와 IP 원문은 저장하지 않고 별도 서버 비밀과 도메인 분리 HMAC-SHA-256으로 생성한 키만 `login_throttles` 테이블에 저장합니다.
